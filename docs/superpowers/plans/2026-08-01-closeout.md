...
## Literature survey result — carry into the pre-registration's limitations section

Searched the 111-paper corpus for a public dataset pairing GD&T tolerances with
assemblability ground truth. **None exists.** This is evidence, not an admission — it is
what justifies building a generator, and it should be stated as a survey result rather
...
| ASSEMCAD / ASSEMBENCH (2607.05123) | ✓ curated mechanical assemblies | ✗ — "tolerance" occurs twice, once as *"tolerance ε along the shortest-gap direction to create volumetric overlap before union"*, a mesh epsilon | ✗ |
| politopix (1509.08763) | GD&T polytopes | ✓ | academic, unmaintained |

Assemblies exist. Tolerances exist. Assemblability judgments exist. **No public dataset has
all three**, which is precisely the gap this work fills.

**Optional Phase 4 extension, worth recording now so it is a planned extension rather than a
reviewer's discovery:** AutoMate's BREP assemblies can serve as *geometry*, with our
tolerance schema applied on top. It does not solve oracle independence — the verdicts would
still be ours plus TolAnalyst — but it answers the reviewer criticism the current corpus is
exposed to, that the geometry is two synthetic plates with holes in a line. Real, public,
large-scale CAD from real designs. Not scheduled here; noted as the natural generalisation
step.
...
Executed plan. Most of its apparently-stale figures are **not** stale: lines ~309, ~419,
~724, ~735 and ~780 quote `tested=11`, `{0.9091, 1.0}` and `"6 PASS"` as *the defects the
tasks were commissioned to fix*, and line ~940 lists the contested quantities *as the input*
to the reconciliation. Those are correct in their role and are left alone.

Two figures are genuinely superseded, both the same one:

- **`eleven` historical instances**, at line ~42 (R5, *"Zero of eleven historical instances
  were found by the three-layer machinery"*) and line ~1116 (the same sentence, as the
  evidence for prioritising N-11). The count is **twelve**. The substance is untouched and
  if anything strengthened — zero of *twelve* were found by the machinery, ten by an
  adversarial reader. Note that the shipped version of R5 in
  `docs/superpowers/specs/2026-08-01-observation-assignment.md` already drops the numeral
  and reads *"Zero of the historical instances"*, which is the form to prefer: refer to
  instances by name, not by number.
