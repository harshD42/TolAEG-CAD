...
def test_virtual_condition_dispatch_uses_equality_not_identity():
    ...
    "virtual_condition" here could accidentally share identity with the
    literal in checker.py even under an `is` mutant. Build the string at
    runtime so identity cannot coincide by accident.
    """
    kind = "".join(["virtual_condition"])
    verdict = check({
        "type": kind,
...
def test_iso_fit_dispatch_uses_equality_not_identity():
    """Same reasoning as above, for the iso_fit branch."""
    kind = "".join(["iso_fit"])
    verdict = check({
